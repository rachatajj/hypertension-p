...
app.config['MYSQL_USER'] = 'b912da2e042f20'
app.config['MYSQL_PASSWORD'] = '0f32689d'
app.config['MYSQL_DB'] = 'heroku_9f8385ea2ed3926'
...
def manageModel(name):
    ...
    on = 'เปิดใช้งาน'
    off = 'ปิด'
    cur.execute("UPDATE `heroku_9f8385ea2ed3926`.`model` SET `status` = %s  WHERE (`type` = %s AND `status` = %s) ",(off,type,on))
    mysql2.connection.commit()

    cur.execute("UPDATE `heroku_9f8385ea2ed3926`.`model` SET `status` = %s  WHERE (`nameModel` = %s) ",(on,name))
    mysql2.connection.commit()
...
def backoffice():
    if session['isAdmin'] == True:
        if request.method == 'POST':
            conn = mysql.connector.connect(host = "us-cdbr-iron-east-04.cleardb.net",user = "b912da2e042f20", passwd = "0f32689d",database = "heroku_9f8385ea2ed3926", port='3306')
...
def changepsswrd():
    ...
    if request.method == 'POST' and form.validate():
        ...
        if sha256_crypt.verify(password_candidate,password):
            cur.execute("UPDATE `heroku_9f8385ea2ed3926`.`user` SET `password` = %s  WHERE (`username` = %s) ",(newpassword,user_id))
            mysql2.connection.commit()
...
def update():
    ...
    if request.method == 'POST':
        ...
        cur = mysql2.connection.cursor()
        cur.execute("UPDATE `heroku_9f8385ea2ed3926`.`user` SET `name` = %s , `date_birth` = %s ,`weight` = %s ,`height` = %s ,`bmi` = %s , `blood_type` = %s WHERE (`username` = %s) ",(name,idate,weight,height,bmi2,blood_type,user_id))
        mysql2.connection.commit()
